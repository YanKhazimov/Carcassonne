...
// ! returns InvalidId in case the initialIds are not all equal, the initialId otherwise
template <typename ... T>
int commonInitialId(const std::shared_ptr<TileObject>& obj0, const T&... args)
{
    return (((obj0->initialId == args->initialId) ? obj0->initialId : InvalidId) & ...);
}
...
Tile::Tile(QJsonObject json, const QString &pathPrefix, QObject *parent)
    : QObject(parent), TileData(json["tileObjectIds"].toArray()),
    imageUrl(QUrl::fromLocalFile(pathPrefix + "/" + json["imageUrl"].toString())),
    imageRotation(json["imageRotation"].toInt()),
    x(json["x"].toInt()),
    y(json["y"].toInt()),
    isFixed(json["isFixed"].toBool()),
    isPlaced(json["isPlaced"].toBool())
{
    QQmlEngine::setObjectOwnership(this, QQmlEngine::CppOwnership);
    // QObject::connect(this, &Tile::layoutChanged, this, &Tile::objectIdsChanged);
...
QJsonObject Tile::serialize() const
{
    return QJsonObject({
        ...
        { "isFixed", isFixed },
        { "isPlaced", isPlaced },
        { "tileObjectIds", TileData::serialize() }
    });
}

// TileObject* Tile::getTileObject(QList<QmlEnums::TileRegion> regions)
// {
//     Q_ASSERT(!regions.empty());
//     qWarning() << regions;
...
//     {
//         switch (regions[i]) {
//         case QmlEnums::NW: objects[i] = NW().get(); break;
//         case QmlEnums::NNW: objects[i] = NNW().get(); break;
//         case QmlEnums::N: objects[i] = N().get(); break;
//         case QmlEnums::NNE: objects[i] = NNE().get(); break;
//         case QmlEnums::NE: objects[i] = NE().get(); break;
//         case QmlEnums::NWW: objects[i] = NWW().get(); break;
//         case QmlEnums::NEE: objects[i] = NEE().get(); break;
//         case QmlEnums::W: objects[i] = W().get(); break;
//         case QmlEnums::C: objects[i] = C().get(); break;
//         case QmlEnums::E: objects[i] = E().get(); break;
//         case QmlEnums::SWW: objects[i] = SWW().get(); break;
//         case QmlEnums::SEE: objects[i] = SEE().get(); break;
//         case QmlEnums::SW: objects[i] = SW().get(); break;
//         case QmlEnums::SSW: objects[i] = SSW().get(); break;
//         case QmlEnums::S: objects[i] = S().get(); break;
//         case QmlEnums::SSE: objects[i] = SSE().get(); break;
//         case QmlEnums::SE: objects[i] = SE().get(); break;
//         default: objects[i] = nullptr; break;
//         }
//     }

//     TileObject* result = objects[0];
//     for (int i = 1; i < objects.size(); ++i)
//     {
//         if (objects[i]->initialId != result->initialId)
//             return nullptr;
//     }
//     return result;
// }
